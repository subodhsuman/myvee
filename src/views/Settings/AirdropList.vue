<template>
  <SettingsLayout>
      <div class="row">
        <div class="col-md-12">
          <div class="exchange_setting_heading">
            <h3 class="fw-bold">Airdrop  Detail</h3>
          </div>
        </div>
      </div>
      ...
      <div class="row mt-2" v-if='this.activityLog'>
        <div class="col-md-12">
          <ul class="list-group">
            <li class="list-group-item mb-3" v-for='(log,index) in activityLog' :key='index'>
...
export default {
  ...
  methods:{
   async callback(page) {
      await ApiClass.getRequest("log/getAirDrop?page=" + page, true).then((res) => {
